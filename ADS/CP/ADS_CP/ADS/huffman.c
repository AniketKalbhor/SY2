...
void write_packed_bits_to_file(char* filename, unsigned char* data, size_t size) {
    FILE* file = fopen(filename, "wb");
    if (!file) {
        fprintf(stderr, "Error opening file %s.\n", filename);
        exit(EXIT_FAILURE);
...
int main(int argc, char *argv[]) {
    ...
    size_t packed_size = (strlen(encoded) + 7) / 8;
    ...
    for (size_t i = 0; i < strlen(encoded); i += 8) {
    ...
    char encoded_file_name[] = "outTemp.bin";
